...
const CountryDetails = () => {
  ...
  return (
    <div className='lightModebg bg-VeryDarkBlue1 text-white min-h-[100vh] '>
      <button onClick={()=>navigate('/')} className='back-button bg-DarkBlue my-5 ms-10 py-3 px-5 shadow-xl rounded-lg focus:border border-White'><i class="ri-arrow-left-line"></i> Back</button>
      <div className='fixed top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%] w-[25%]'>{(loading === true) ? <img src={loadingImage} /> : ''}</div>
      {
        ...
        <div className='p-5 space-y-4 text-content'>
            ...
            <h4><strong className='me-3'>Borders:</strong>{Object.keys(element).includes('borders') &&  (element.borders.map(border=> (
              <div className='borderDiv inline-block ms-4 py-2 px-3 mb-3'>{border}</div>
            )))}</h4>
        </div>
        </div>
        ))
